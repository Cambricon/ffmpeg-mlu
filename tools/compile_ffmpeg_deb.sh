...
echo ""
echo "---------------install aac---------------"
cd ${DEB_DIR}/fdk-aac && ./autogen.sh && ./configure --prefix="${WORKDIR}" --disable-shared && make -j && make install
...
echo ""
echo "--------------install x264---------------"
cd ${DEB_DIR}/x264 && ./configure --prefix="${WORKDIR}" --enable-shared --enable-pic && make && make install

echo ""
echo "--------------install x265---------------"
cd ${DEB_DIR}/x265/build/linux && cmake -G "Unix Makefiles" -DCMAKE_INSTALL_PREFIX=${WORKDIR} -DENABLE_SHARED=on ../../source && make -j && make install
